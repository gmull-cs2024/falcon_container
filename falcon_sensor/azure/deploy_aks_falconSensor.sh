...
install_falcon_sensor() {
  ...
  if [ -z "$ACR_NAME" ]; then
    ...
    echo "Please set it using: export ACR_NAME=<your_acr_name>"
    exit 1
  fi

  if [ -z "$FALCON_SENSOR_REPO" ]; then
    echo "Error: FALCON_SENSOR_REPO environment variable is not set."
    echo "Please set it using: export FALCON_SENSOR_REPO=<acr_repo_path>"
    exit 1
  fi
  ...
  # Construct the full ACR login server and image repository path
